...
// Friendly URL: /news/123-tieu-de-bai-viet
if (empty($do) && empty($id)) {
    $requestPath = parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH);
    $newsBase = rtrim(dirname(dirname($requestPath)), '/');
    $parts = array_values(array_filter(explode('/', trim($requestPath, '/'))));
    $newsPos = array_search('news', $parts, true);
    if ($newsPos !== false && isset($parts[$newsPos + 1]) && preg_match('/^(\d+)(?:-[^/]*)?$/', $parts[$newsPos + 1], $match)) {
        $id = (int) $match[1];
        $do = 'view';
    }
}

function johncms_news_slug($title, $id = 0)
{
...
// Canonicalize legacy article URLs to the friendly URL.
if ($do === 'view' && $id && strpos(parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH), '/news/') !== false) {
    $canonical = $db->query("SELECT `id`, `name` FROM `news` WHERE `id` = " . (int) $id . " LIMIT 1")->fetch();
    if ($canonical) {
        $path = parse_url($_SERVER['REQUEST_URI'] ?? '', PHP_URL_PATH);
        $expected = '/news/' . johncms_news_slug($canonical['name'], (int) $canonical['id']);
        if (rtrim($path, '/') !== rtrim($expected, '/')) {
            header('Location: ' . $config->homeurl . '/news/' . johncms_news_slug($canonical['name'], (int) $canonical['id']), true, 301);
            exit;
        }
    }
}
...
switch ($do) {
    ...
    case 'view':
        // Đọc một bài viết riêng trên giao diện Blog
        if (!$id) {
            header('Location: index.php');
            exit;
        }

        $res = $db->query("SELECT * FROM `news` WHERE `id` = " . (int) $id . " LIMIT 1")->fetch();
        if (!$res) {
            echo $tools->displayError(_t('Wrong data'), '<a href="index.php">' . _t('Back to news') . '</a>');
            break;
        }

        echo '<article class="article-view">';
        echo '<a class="article-back" href="../">← ' . _t('Home') . '</a>';
        echo '<h1>' . htmlspecialchars($res['name'], ENT_QUOTES, 'UTF-8') . '</h1>';
        echo '<div class="article-meta">' . $res['avt'] . ' · ' . $tools->displayDate($res['time']) . '</div>';
        echo '<div class="article-content">' . $tools->checkout($res['text'], 1, 1) . '</div>';

        if ($res['kom'] != 0 && $res['kom'] != '') {
            $komm = $db->query("SELECT COUNT(*) FROM `forum` WHERE `type` = 'm' AND `refid` = '" . (int) $res['kom'] . "'")->fetchColumn();
            echo '<a class="article-discuss" href="../forum/?id=' . (int) $res['kom'] . '">' . _t('Discuss in Forum') . ' (' . (int) $komm . ')</a>';
        }
        echo '</article>';
        break;
